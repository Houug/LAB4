...
void Doctor::inputSpecialtyId()
{
	do
	{
		cout << "Введите специальность (1-3): ";
		cin >> specialty;
	} while (checkInput() && checkInput(specialty, 0));
}
...
void Doctor::inputQualificationId()
{
	do
	{
		cout << "Введите квалификацию (0-2): ";
		cin >> qualification;
...
void Doctor::printDoctor(int n)
{
	...
	Fio fio = this[n].getFIO();
	cout << setw(40) << fio << setw(16) << specialty << setw(16) << qualification << endl;
}
